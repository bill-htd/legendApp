...
class Role extends EntityModel {
	...
	public parser(bytes: GameByteArray): void {
		...
		//这里是合并版本的时候为了和后端对上协议增加,其龙城内容为第二套技能.
		for (i = 0; i < 5; i++) {
			bytes.readInt();
		}
		this.yuanshengData = [];
		...
		this.parserhHeirloom(bytes);//传世装备
		this.parserhWeapons(bytes);//兵魂
	...
	/**
	 * 通过锻造类型获取等级最小的装备索引
	 * @param type 0 强化 1 铸造 2 精炼  突破和兵魂是两个不同功能 突破是原来旧系统的
	 */
		...
		for (let n: number = 0; n < num; ++n) {
			...
			switch (type) {
				...
				case ForgeWin.Page_Select_ZhuLing://精炼
					...
					break;
				case 3://这里是原有的突破功能 兵魂是单独处理 所以此3非兵魂的3
	...
	public getWeaponTotalPower(id?: number) {
		...
		// 	}
		// }
		let index = this.weapons.flexibleCount ? this.weapons.flexibleCount - 1 : 0;
		...
		if (attr.length && config) {
			for (let i = 0; i < config.attr.length; i++) {
				...
				if (!isHave) {
					let at: AttributeData = new AttributeData;
					let tmp: { type: number, value: number } = config.attr[i];
					at.type = tmp.type;
					at.value = tmp.value;
					attr.push(at);
				}
	...
	public get guildAndName(): string {
		let nameStr = this.getNameWithServer();
		let specailColor: string = this.camp > 0 && BattleCC.ins().isBattle() ? (this.camp != BattleCC.ins().camp ? "#FF0000" : "#00FF00") : null;
		let str: string = this.guildID == 0 || this.guildID == undefined ? nameStr : `<font color='${specailColor ? specailColor : this.guildID == GuildWar.ins().getModel().winGuildInfo.guildId ? "#ffb243" : this.guildID == Guild.ins().guildID ? "#5add70" : "#2CC2F8"}}'>` + this.guildName + "</font>" + "\n" + this.name;
		if (this.lilianLv > 0) {
			let config: TrainLevelConfig = GlobalConfig.TrainLevelConfig[this.lilianLv];
			if (config.type > 1) {
				let color: string = ColorUtil.JUEWEI_COLOR[config.type - 1] || ColorUtil.JUEWEI_COLOR[ColorUtil.JUEWEI_COLOR.length - 1];
	...
	public hideWeapons(): boolean {
		...
		return GlobalConfig.EquipConfig[id] && GlobalConfig.EquipConfig[id].noWSoulEff >= 1;  //是否隐藏兵魂特效
	}
