...
class UserVip extends BaseSystem {
	...
	public vipGiftState:{id:number,state:number}[] = []; /** 领取状态 （按位表示 0000101表示1级和3级的已领取）*/
	...
	protected initLogin(){
		...
		// if( !this.vipGiftState || !this.vipGiftState.length ){
		// 	this.vipGiftState = [];
		// 	let maxId = Object.keys(GlobalConfig.VipGiftConfig).length;
		// 	for (let id = 1; id <= maxId; id++) {
		// 		this.vipGiftState.push({id:id,state:0});
		// 	}
		// }
	}
	...
	public postUpdataExp(bytes: GameByteArray): void {
		...
		let newExp = bytes.readInt();
		let successYuanBaoNum = newExp - this.exp
		this.exp = newExp
