...
class KfArenaInviteWin extends BaseEuiView {
	...
	public open(...args: any[]): void {
		...
		//this.addTouchEvent(this.teamBtn, this.onTouch);
		//this.addTouchEvent(this.friendBtn, this.onTouch);
	...
	private onTouch(e: egret.TouchEvent): void {
		switch (e.target) {
			case this.closeBtn0:
				...
				break;
			// case this.teamBtn:
			// 	this.setType(KFInviteType.Guild);
			// 	break;
			// case this.friendBtn:
			// 	this.setType(KFInviteType.Friend);
			// 	break;
	...
	public update(id: number): void {
		for (let i = this.friendList.length - 1; i >= 0; i--) {
			if (this.friendList[i].id == id)
				this.friendList.splice(i, 1);
		}
		for (let i = this.guildList.length - 1; i >= 0; i--) {
