class HejiEquipTipsWin extends BaseEuiView {
	...
	private setData(_data?: any): void {
		...
		if (!(_data instanceof ItemData)) {
			...
			let itemData:ItemData = UserBag.ins().getBagGoodsByTypeAndId(UserBag.BAG_TYPE_EQUIP,data.configID);
			...
			if( next ){
				let itemData:ItemData = UserBag.ins().getBagGoodsByTypeAndId(UserBag.BAG_TYPE_EQUIP,next.id);
				if( itemData )
	...
	private gainWay: any[] = [["参与击杀秘境BOSS","BossWin",2],["限时任务","LimitTaskView"]];
	private unactivated(data:any){
		if( this.currentState == "max" || this.currentState == "default" )return;
		...
		let itemData:ItemData = UserBag.ins().getBagGoodsByTypeAndId(UserBag.BAG_TYPE_EQUIP,nextItemConfig.id);
		if( itemData ){
	...
	private setNextAttrs(data:any){
		...
		this.score.visible = false;//合击不显示评分
		if (data.att) {
		...
		if(config.baseAttr1){
			let att:AttributeData = new AttributeData(config.baseAttr1.type, config.baseAttr1.value);
			totalAttr.push(att);
		}

		if(config.baseAttr2){
			let att:AttributeData = new AttributeData(config.baseAttr2.type, config.baseAttr2.value);
			totalAttr.push(att);
		}
