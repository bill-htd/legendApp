...
class UserBag extends BaseSystem {
	...
	static _fitleEquip:number[];

	// //装备数量
	// public equipCount:{[id:number]:number} = {};
	...
	public constructor() {
		...
		this.regNetMsg(8, this.doChoosableGiftResult);
			...
			if (addNum > 0) {
				if (type != 2) {
					if( ItemConfig.getType(element.itemConfig) == ItemType.TYPE_18 ){
						...
						let quality: number = ItemConfig.getQualityColor(element.itemConfig);
						let str: string = "获得|C:" + quality + "&T:" + element.itemConfig.name + " x " + addNum + "|";
						UserTips.ins().showTips(str);
					}
					UserBag.ins().postItemAdd();
				}
	...
	/**派发是否有可用道具提示*/
	public postHasItemCanUse(): number {
	 ...
	 * @param type 类型
	 */
	public getBagEquipByType(type: number) {
	...
	public getBagByType(t:number):ItemData[] {
	 ...
	 * @param id
	 */
	public getBagGoodsCountByType(type: number, ids?: number[]): boolean {
	 ...
	 * @param id
	 */
	public getRuneBagGoodsCountByType(type: number): boolean {
		let itemData: ItemData[] = this.bagModel[UserBag.BAG_TYPE_OTHTER];
		if (itemData) {
			for (let i: number = 0; i < itemData.length; i++) {
				if (ItemConfig.getType(itemData[i].itemConfig) == 8) {
	 ...
	 * @param index
	 */
	public getBagGoodsByIndex(type: number, index: number): ItemData {
		return this.bagModel[type][index];
	}

	/**
	 * 获取背包道具数量
	 * @param type    0是其他物品 1是装备 2是战纹/戒指等(具体看表或者问策划)
	 */
	...
	public getLegendOutEquips() {
		let list: ItemData[] = UserBag.ins().getBagEquipsByQuality(5);
		let returnList: ItemData[] = [];
		let nowEquips: any[] = [];
		let roleList: any[] = [];
		//穿在身上的装备
	...
	public getLegendHasResolve(): boolean {
		let list: ItemData[] = UserBag.ins().getBagEquipsByQuality(4);
		let returnList: ItemData[] = [];
		let nowEquips: any[] = [];
		...
		for (let i: number = 0; i < list.length; i++) {
			...
			// 	return true;
			// }
			if( BreakDownView.fitleEquip[item.configID] )continue;//轮回装备不提示分解
	 ...
	 * type  0  普通物品背包     1战纹背包
	 */
	public getBagGoodsBySort(type: number) {
	 ...
	 * @param type 类型
	 */
	public getBagGoodsByType(type: number) {
	 ...
	 * @param descIndex 类型
	 */
	public getBagGoodsByDescIndex(descIndex: number, Bag_Type: number = UserBag.BAG_TYPE_OTHTER): ItemData {
		let itemData: ItemData[] = this.bagModel[Bag_Type];
		let itemData1: ItemData;
	 ...
	 * @param type 类型
	 */
	public getBagEquipsByQuality(quality: number, type: number = 0, Bag_Type: number = UserBag.BAG_TYPE_EQUIP) {
		let itemData: ItemData[] = this.bagModel[Bag_Type];
		let itemData1: ItemData[] = [];
		for (let a in itemData) {
	 ...
	 * @param handle
	 */
	public getBagGoodsByHandle(type: number, handle: number): ItemData {
		let itemData: ItemData[] = this.bagModel[type];
		for (let i: number = 0; i < itemData.length; i++) {
	...
	public getGuanyinLevel(itemConfig): string {
		let str = TextFlowMaker.getCStr(itemConfig.id - ItemConst.GUANYIN_ITEM + 1);
		return `${str}阶`;
	}
	...
	/** 计算所有角色身上的装备评分总和 **/
	public getEquipsScoreByRolesOfBody(){
		let len:number = SubRoles.ins().subRolesLen;
		let point:number = 0;
		for( let i = 0;i < len;i++ ){
			let role:Role = SubRoles.ins().getSubRoleByIndex(i);
	...
	//选择评分低的装备熔炼
	public checkHejiEquipsTodestroy(returnList: ItemData[]): void {
	 ...
	 * 可选礼包使用结果
	*/
	private doChoosableGiftResult(bytes:GameByteArray):void
	{
		//0成功
		//非0失败
