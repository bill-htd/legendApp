...
class TitleItem extends BaseItemRender{
	...
	public dataChanged():void
	{
		...
		if(this.data instanceof TitleInfo)
		{
			...
			if(titleInfo.config.eff){
				...
				let eff = RES_DIR_EFF+titleInfo.config.eff;
			...
			{
				if(this._power)
					BitmapNumber.ins().desstroyNumPic(this.power.getChildAt(0) as egret.DisplayObjectContainer);
	...
	private onRemove(){
		...
		WatcherUtil.removeFromArrayCollection(this.attrs.dataProvider as eui.ArrayCollection);
		WatcherUtil.removeFromArrayCollection(this.attrsTotal.dataProvider as eui.ArrayCollection);
