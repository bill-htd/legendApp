class Book extends BaseSystem {
	...
	public getUpChipData(booKData: BooKData): number[] {
		...
		for (let k in this.listBook) {
			...
			let count = UserBag.ins().getBagGoodsCountById(0, data.itemId);
	...
	public checkResolveRedPoint() {
		let itemData = UserBag.ins().getBagGoodsByType(ItemType.TYPE_9);
		if (!itemData.length) {
			return false;
		}
	...
	public getSuitRedPoint(s) {
		...
		let books = UserBag.ins().getBagGoodsByType(ItemType.TYPE_9);
