...
class Wing extends BaseSystem {
	...
	public constructor() {
		...
		this.observe(UserBag.ins().postItemChange, this.startCheckHaveCan);
		this.observe(this.postUseDanSuccess, this.startCheckHaveCan);
	}
		...
		for (let i: number = 0; i < len; i++) {
			...
			if (roleOpenWing) {
				...
				if (curlevel < lvMax && curlevel < Wing.WingExpRedPoint) {
					...
					let num: number = UserBag.ins().getBagGoodsCountById(0, config.itemId);
				...
				//翅膀直升丹
				if( !boolList[i] ){
					let count: number = UserBag.ins().getBagGoodsCountById(0,GlobalConfig.WingCommonConfig.levelItemid);
	...
	public isHaveActivationWing(roleId: number = -1): boolean {
		let myLevel: number = Actor.level;
		let needLevel: number = GlobalConfig.WingCommonConfig.openLevel;
		if (myLevel < needLevel) return false;
	...
	*/
	...
	public getWearItem(roleId:number,slot:number){
		let lv:number = Wing.ins().getGodWing(roleId).getLevel(slot);
		let items:ItemData[] = UserBag.ins().getBagGoodsByType(ItemType.TYPE_16);
