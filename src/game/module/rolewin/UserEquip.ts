class UserEquip extends BaseSystem {
	...
	public constructor() {
		...
		this.observe(UserBag.ins().postItemChange, this.delayCheckHaveCan);//道具变更
		this.observe(UserBag.ins().postItemAdd, this.delayCheckHaveCan);//道具添加
		this.observe(UserBag.ins().postItemDel, this.delayCheckHaveCan);//道具删除
		this.observe(UserBag.ins().postHuntStore, this.delayCheckHaveCan);
		this.observe(Actor.ins().postLevelChange, this.delayCheckHaveCan);
	}
	...
	private delayCheckHaveCan() {
		if (!TimerManager.ins().isExists(this.postCheckHaveCan, this)) TimerManager.ins().doTimer(60, 1, this.postCheckHaveCan, this);
	}
	...
	/**用于RoleWin的canEquip判定神装or热血装备可合成时候的红点(增加碎片判定)*/
	public checkRedPointEx(qualty: number, roleId?: number): boolean {
		let itemList = UserBag.ins().getBagEquipsByQuality(qualty);
		...
		for (let i: number = 0; i < len; i++) {
			...
			if (fitL > L) {
				if(qualty == 4){//走神装检查是否有足够碎片可合成 需要进一步判定碎片是否足够
					let curNum = UserBag.ins().getBagGoodsCountById(0, id);
					let comConfig:LegendComposeConfig = GlobalConfig.LegendComposeConfig[id];
					let needNum = comConfig?comConfig.count:0;
					if (curNum >= needNum)
						return true;
				}else{
					return true;
	...
	/**用于RoleInfoPanel的setRetPoint热血装备红点判定*/
	public checkRedPointExOrg(qualty: number, roleId?: number): boolean {
		let itemList = UserBag.ins().getBagEquipsByQuality(qualty);
		let len = itemList.length;
		if( len ){
	...
	private IsEnoughChip(roleId:number){
		let arr = [EquipPos.WEAPON,EquipPos.CLOTHES];
		let role = SubRoles.ins().getSubRoleByIndex(roleId);
		for( let j = 0;j < arr.length;j++ ){
			let mixConfig = this.getMixConfig(role.index,arr[j]);
			if( !mixConfig )continue;
			let needNum = mixConfig.count;
			let costID = mixConfig.itemId;
	...
	private getMixConfig(roleId:number,pos:number){
		let role = SubRoles.ins().getSubRoleByIndex(roleId);
		let configId: number = this.getEquipConfigIDByPosAndQualityByLegend(role.index, pos, 5);
	...
	public setLegendEquipItemUpState(index: number, role: Role): boolean {
		...
		if (nextEquipData == undefined && equipData.item.handle != 0 && q == 5) {
			...
			if (nextEquipData != undefined && equipData.item.handle != 0 && q == 5) {
				...
				let curNum = UserBag.ins().getBagGoodsCountById(0, grewupConfig.itemId);
				//背包数量对比
