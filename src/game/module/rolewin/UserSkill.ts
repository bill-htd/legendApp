class UserSkill extends BaseSystem {
	...
	public static ACTIVE_LEVEL: number = 10; //激活关卡数
	public static HEJI_SKILL: number = 70000;  //合击技能ID
	public static HEJI_SHOW_LEVEL: number = 60; //合击进入装备等级
	...
	public constructor() {
		...
		this.regNetMsg(2, this.postDoBuff);
		this.regNetMsg(3, this.postDoRemoveBuff);
	 ...
	 * @param bytes
	 */
	public postDoBuff(bytes: GameByteArray): void {
	 ...
	 * @param bytes
	 */
	public postDoRemoveBuff(bytes: GameByteArray): void {
		let handle: number = bytes.readDouble();
		let char: CharMonster = EntityManager.ins().getEntityByHandle(handle) as CharMonster;
		if (char) {
	...
	*/
	...
	public isHejiSkill(id):boolean {
		for (let i in GlobalConfig.TogetherHitConfig) {
...
class SkillConst {
	public static EFF_SKY_BALL: number = 53001;//官印技能

	public static HEJI_SKILLS:number[] = [71001,72001,73001];
}