class UserRole extends BaseSystem {
	...
	public constructor() {
		...
		this.observe(GameLogic.ins().postSubRoleChange, this.startCheckHaveCan);
		this.observe(UserEquip.ins().postEquipChange, this.startCheckHaveCan);
		this.observe(JadeNew.ins().postJadeData, this.startCheckHaveCan); //玉佩信息
		this.observe(LyMark.ins().postMarkData, this.startCheckHaveCan);
		this.observe(Wing.ins().postUseDanSuccess, this.startCheckHaveCan);
		this.observe(Wing.ins().postWingUpgrade, this.startCheckHaveCan);

		this.observe(Actor.ins().postGoldChange, this.delayShowNavBtnRedPoint);
		this.observe(GameLogic.ins().postRuneExchange, this.delayShowNavBtnRedPoint);
		this.observe(GameLogic.ins().postRuneShatter, this.delayShowNavBtnRedPoint);
		this.observe(UserBag.ins().postHuntStore, this.delayShowNavBtnRedPoint);
		this.observe(UserFb.ins().postFbRingInfo, this.delayShowNavBtnRedPoint);
	}
	...
	public checkHaveCan(isWear: boolean = false, roleIndex: number = -1): void {
		this.timeID = 0;
		//背包装备
		let equipItems: ItemData[] = UserBag.ins().getBagEquipByType(0);
		if (!equipItems)
			return;

		let sr = SubRoles.ins();

		let startIndex = roleIndex >= 0 ? roleIndex : 0;
		let endIndex = roleIndex >= 0 ? roleIndex + 1 : sr.subRolesLen;

		let tempChangeEquips: ItemData[][] = [[], [], []];

		let slen = sr.subRolesLen;
		for (let i = 0; i < slen; i++) {
			for (let equip of sr.getSubRoleByIndex(i).equipsData) {
	...
	private delayShowNavBtnRedPoint() {
		if (!TimerManager.ins().isExists(this.showNavBtnRedPoint, this)) TimerManager.ins().doTimer(60, 1, this.showNavBtnRedPoint, this);
	}
	...
	public showNavBtnRedPoint(b: boolean = false): void {
		...
		if (!b) b = SamsaraModel.ins().isCanUpgrade() || SamsaraModel.ins().isCanExchange() || SamsaraModel.ins().isCanUpgradeSoul();//可以升级或者可以兑换时候人物图标有红点
		//玉佩
		if (!b)
