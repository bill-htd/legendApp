class TreasureChuanshiPanel extends BaseView {
	...
	private buyHunt(type: number): void {
		...
		this.huntType = type;
		let item: ItemData = UserBag.ins().getBagGoodsByTypeAndId(UserBag.BAG_TYPE_OTHTER, GlobalConfig.HeirloomTreasureConfig.huntItem);
		if (item && item.count) {
			Heirloom.ins().sendHunt(type);
		}
		else {
	...
	private huntWarnFun(): void {
		let huntOnce: number = this.huntType == 0 ? GlobalConfig.HeirloomTreasureConfig.huntOnce : GlobalConfig.HeirloomTreasureConfig.huntTenth;
	...
	private maskImage() {
		...
		let percent = Math.floor(0.01 * Math.pow(curPross, 0.74) * maxPross) / maxPross;//显示百分比
