...
class HeartMethod extends BaseSystem {
    ...
    public getHeartSlotItemIdWear(roleId: number, hearId: number, pos: number): number {
        ...
        let hmdata: HearMethodData = hmdataMap[hearId];
        if (!hmdata)return 0;
        let itemData: ItemData[] = UserBag.ins().getBagGoodsByType(ItemType.TYPE_18);
        itemData.sort(this.changeSort);//从好到坏
        for (let k in itemData) {
            let cfg: HeartMethodStarConfig = GlobalConfig.HeartMethodStarConfig[itemData[k].configID];
            if (cfg.heartmethodId != hearId)continue;
            let itemConfig: ItemConfig = itemData[k].itemConfig;
            ...
            let pId: number = this.getSuitPosFromItemId(itemData[k].configID);
            if (pos == pId)//找到对应可穿部位
    ...
    public calcHeartSlotChange(roleId: number, hearId: number, itemid: number): number {
        ...
        itemData.sort(this.changeSort);//从好到坏
        for (let k in itemData) {
            if (GlobalConfig.HeartMethodStarConfig[itemData[k].configID].posSort != pId)continue;//不是同一个部位的忽略
            if (GlobalConfig.HeartMethodStarConfig[itemData[k].configID].heartmethodId != hearId)continue;//不是同一个心法的忽略
            let itemConfig: ItemConfig = GlobalConfig.ItemConfig[itemData[k].configID];
            let lv = itemConfig.level ? itemConfig.level : 0;
            let zslv = itemConfig.zsLevel ? itemConfig.zsLevel : 0;
            if (Actor.level < lv || UserZs.ins().lv < zslv)continue;//至少要符合穿戴条件
            //和身上的对比
