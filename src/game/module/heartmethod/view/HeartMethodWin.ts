...
class HeartMethodWin extends BaseView {
			...
			if( e.currentTarget == this[`item${i+1}`] ){
				...
				if( !slotId ){
					...
					for( let k in itemData ){
						let sconfig:HeartMethodStarConfig = GlobalConfig.HeartMethodStarConfig[itemData[k].configID];
	...
	private updateHeart(){
		...
		if( hmdconfig && hminfo && hminfo[id] ){
			...
			// let stageLevel:number = hminfo[this.heartId].stage;
			if (lv > 0) {
	...
	private updateRedPoint(){
		...
		// 	if( GlobalConfig.HeartMethodStarConfig[itemData[i].configID].heartmethodId == id ){
