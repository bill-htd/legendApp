class NewArtifactWin extends BaseEuiView {
	...
	public constructor() {
		...
		// this.setSkinPart("pieceCom", new ShenqiSelectItemWin());
		// this.setSkinPart("powerPanel0", new PowerPanel());
		// this.setSkinPart("shenqispTipsWin", new shenqispTipsWin());
	...
	private updateView() {
		...
		if (data.open && this.isBtnOpen) {
			...
			Activationtongyong.show(0, conf.name, `j${img}_png`, true, () => {
				if (conf.winGuide) {
