...
class RuneDecomposePanel extends BaseView {
	...
	private materialLab: eui.Label;
	private addMaterialLab: eui.Label;
	...
	public onDecomposeResult(param: any[]): void {
		if (param[0]) {
			...
			// UserTips.ins().showTips(`分解成功，获得${num}个${str}战纹精华`);
			if (this._chipNum)
	...
	private decomposeRune(): void {
		...
		if (this.cardList && this.cardList.length > 0) {
			...
			if (hasMaxQuality) {
				...
				let plusStr: string = "";
				// if (hasSpecail) specialStr = "<font color = '0xFFFFFF' size = '22'>[特殊战纹]</font>";
				if (hasMaxQuality) qualityStr = "<font color = '" + ItemBase.QUALITY_COLOR[RuneConfigMgr.ins().getOtherCfg().maxQuality] + "' size = '22'>顶级战纹</font>";
				// if (hasSpecail && hasMaxQuality) plusStr = "和";

				let str: string = specialStr + plusStr + qualityStr;
				let finalStr: string = "当前分解战纹包括" + str + "，是否继续进行分解";
	...
	public updateAddShatter(): void {
		let addNum: number = 0;//战纹精华
		let chip: number = 0;//战纹碎片
		let materiaCount: number = 0;
		let rbc: RuneBaseConfig = null;
		let materialName: string = GlobalConfig.ItemConfig[GlobalConfig.RuneOtherConfig.goldItemId].name;
			...
			for (let v of this.cardList) {
				if (v && v.isSelected && v.itemData && v.itemData.itemConfig) {
					...
					if (rbc) {
						...
						if (rbc.chip)
							chip += rbc.chip;
						let config = GlobalConfig.RuneComposeConfig[rbc.id];
						if (config) {
	...
	private initCardList(): void {
		this.cleanCardList();
		this.cardList = [];
		let list: ItemData[] = UserBag.ins().getBagGoodsByType(6);
