...
class RuneReplaceWin extends BaseEuiView {
	...
	private onListTap(e: egret.Event): void {
		...
		if (item && item instanceof ItemData) {
			...
			let arrStr: string[] = Rune.ins().getMergeRune(item.configID, this.roleIndex, this.item);
			if (arrStr) {
				WarnWin.show(arrStr[0], () => {
	...
	private updateList(): void {
		let itemDataList: ItemData[] = UserBag.ins().getBagGoodsByType(6);
		if (itemDataList) {
			//数据
			// let datas: ItemData[] = [];
