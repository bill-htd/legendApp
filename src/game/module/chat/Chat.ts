class Chat extends BaseSystem {
	...
	public canSpeakTime:number = 0;//可以聊天的时间戳 单位秒
	...
	public constructor() {
		...
		this.regNetMsg(5, this.doCannotSpeak);
	}
	...
	public sendChatInfo(type: number, str: string, pointId: number = 0): void {
		...
		if (type == 7 && this.isCanSpeak()) {//世界聊天
	 ...
	 * @param bytes
	 */
	private doCannotSpeak(bytes: GameByteArray) {
		this.canSpeakTime = bytes.readInt();
	}

	//是否禁言中
	public isCanSpeak():boolean {
