...
class GodWeaponCC extends BaseSystem {
	...
	private _timeMijinAry: number[];//秘境的评分时间
	...
	public godWeaponIsOpen() {
		...
		// return this.weaponDataAry && this.weaponDataAry.length > 0;
	}
	...
	public mijintHadRedPoint(): boolean {
		if (this.godWeaponIsOpen() && this.fubenInfoData && this.fubenInfoData.hadChallengeNum > 0) {//神兵副本还有免费的挑战次数
			return true;
		} else {
			return false;
		}
	...
	public gwAddAttr(weaponId: number): { value: number, type: number }[] {
		let weaponData = this.getWeaponData(weaponId + 1);
		if (!weaponData) return [];
		let gwAry: { value: number, type: number }[] = weaponData.addAttr.concat();//神兵等级属性加成
