...
class GwWeaponView extends BaseView {
	...
	private power: BaseComponent;//战力
	private detailBtn: eui.Image;//详
	private flameMC: MovieClip;//火
	private powerCon: egret.DisplayObjectContainer;
	...
	private updatePower(): void {
		let ary: { value: number, type: number }[] = GodWeaponCC.ins().gwAddAttr(this.selectIndex);
		...
		if (GodWeaponCC.ins().weaponIsActive(this._selectIndex + 1)) {
			...
			let max: number = GodWeaponCC.ins().maxSkillIdAry[this.selectIndex];
	...
	private touchHandler(e: egret.TouchEvent): void {
		if (e.target instanceof eui.Button) {
			...
			let dataWeapon: GodWeaponData = GodWeaponCC.ins().getWeaponData(this.selectIndex + 1);
...
class GwGodWeaponSkillView extends BaseView {
	...
	public set selectIndex(weaponId: number) {
