class SpecialRing extends BaseSystem {
	...
	private postRingAbility(bytes: GameByteArray): void {
		...
		// this.abilityId = bytes.readShort();
		...
		for (let id in dp) {
			if (dp[id][1]) {
				let item = UserBag.ins().getBagItemById(dp[id][1].itemId);
				if (item) item.setCanbeUsed();
			}
		...
		if (id == SpecialRing.FIRE_RING_ID && this.isFireRingFuse()) {//需要融合之后才能升级
			...
			if (this.getRingConfigById(id, level + 1)) {//需要有下一阶可以升级
				if (level % 11 == 0) {
					...
					let count: number = UserBag.ins().getBagGoodsCountById(0, config.costItem);
	...
	public isFireRingCanActivate(): boolean {
		...
		if (SpecialRing.ins().loginDayCount >= config.openDay) {//解封条件之一 需要登录时间
			for (let i: number = 0; i < this.ringList.length; i++) {
				canActivate = true;
				if (this.ringList[i].id != SpecialRing.FIRE_RING_ID && this.ringList[i].level == 0) {//解封条件二，需要其他特戒激活
					canActivate = false;
					break;
				}
			}
	...
	public isCanUpgradeSkill(): boolean {
		...
		for (let i = 0; i < count; i++) {
			let ring: RingSkillInfo = this.skillInfo[i];
			if (ring.skillId > 0 && ring.skillLvl < this.getSkillMaxLvl(ring.skillId)) {
	...
	public getRingSkill(): number {
		...
		if (lvl > 0 && skillId) {
			...
			let prefix = Math.floor(skillId / 10);
