...
class BoostPowerView extends BaseEuiView {
	...
	public initUI(): void {
		...
		this.imgBg.source = "zhandoulibg";//RES.getRes("zjm_48");
		this.imgBg.x = 50;
		this.imgBg.y = 438;
		...
		this.img.source = "zjmzhandouli";//RES.getRes("zjm_48");
		this.img.x = 100;
		this.img.y = 450;
	...
	private showPowerUp(lasterPower: number, nowPower: number) {
		...
		this.imgBg.alpha = 1;
		let currentPower: egret.DisplayObjectContainer = BitmapNumber.ins().createNumPic(lasterPower, "8", 5);
		currentPower.y = -17;
		...
		TimerManager.ins().doTimer(20, 25, () => {
			...
			let boostPower: egret.DisplayObjectContainer = BitmapNumber.ins().createNumPic(numStr, "r0", 5);
