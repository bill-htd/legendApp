...
class TipsView extends BaseEuiView {
	...
	private taskTipsList: CenterTipsItem[] = [];
	 ...
	 * @param str
	 */
	public showCenterTips3(str: string): void {
		let tips: CenterTipsItem = ObjectPool.pop("CenterTipsItem");
		tips.verticalCenter = 0;
		tips.horizontalCenter = 0;
		tips.labelText3 = str;
	...
	private removeCenterTip3(e: egret.Event): void {
		let tips = e.currentTarget;
		let index: number = this.centerList.indexOf(tips);
		this.centerList.splice(index, 1);
		tips.horizontalCenter = NaN;
	...
	public showTaskTips(str: string) {
		let tips: CenterTipsItem = ObjectPool.pop("CenterTipsItem");
		tips.bottom = 232;
		tips.horizontalCenter = 0;
		tips.labelText = str;
		...
		for (let i: number = this.taskTipsList.length - 1; i >= 0; i--) {
			egret.Tween.removeTweens(this.taskTipsList[i]);
			let t: egret.Tween = egret.Tween.get(this.taskTipsList[i]);
			t.to({"bottom": 232 + i * 30}, 500);
		}
	}

	private removeTaskTip(e: egret.Event): void {
		let index: number = this.taskTipsList.indexOf(e.currentTarget);
	...
	private equipHeartItemList: any[] = [];
	private equipHeartTip: TipsHeartEquipAlertPanel;
	...
	public showHeartItemTip(item: number) {
	...
	private itemHeartTipTimer() {
		...
		let itemTip: TipsHeartEquipAlertPanel;
		...
		let t: egret.Tween = egret.Tween.get(itemTip);
		t.to({"y": 600 - 37}, 300).wait(1000).to({"alpha": 0}, 1000).call(() => {
			itemTip.isUsing = false;
			this.isWait = false;
			this.removeChild(itemTip);
		});
