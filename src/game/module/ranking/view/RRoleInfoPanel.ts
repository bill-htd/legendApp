...
class RRoleInfoPanel extends BaseView {
	...
	public juesexiangxi: eui.Button;
	...
	protected setEquip(role: Role): void {
		...
		for (let i = 0; i < len; i++) {
			...
			// this.equips[i].setCurRole(role.index);
	...
	private playZhanLingAttack() {
		if (this.warMc) {
			let s: string = RES_DIR_MONSTER + this.fileName + "_4" + EntityAction.ATTACK;
			this.warMc.playFile(s, 1, () => {
				let src: string = RES_DIR_MONSTER + this.fileName + "_4" + EntityAction.STAND;
