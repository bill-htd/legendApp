...
class SpringEightDayWin extends BaseEuiView{
	...
	{
		let data:ActivityType11Data = Activity.ins().getActivityDataById(this._activityID) as ActivityType11Data;
		this._curDay = data.getCurDay();

		this._items = [];
		let confs:ActivityType11_2Config[] = GlobalConfig.ActivityType11_2Config[this._activityID];
	...
	private updateData():void
	{
		...
		let state:number = 0;
		let achieve:{times:number, isGot:boolean};
		let activityData:ActivityType11Data = Activity.ins().getActivityDataById(this._activityID) as ActivityType11Data;
	...
	{
		let conf:ActivityType11_1Config = GlobalConfig.ActivityType11_1Config[this._activityID][Object.keys(GlobalConfig.ActivityType11_1Config[this._activityID]).length];
		this.schedule.maximum = conf.score;
		this.schedule.value = (Activity.ins().getActivityDataById(this._activityID) as ActivityType11Data).hiScore;
	}
