class OSATarget9Panel2 extends BaseView {
	...
	public reward0: FireworkerItemRender;
	public reward1: FireworkerItemRender;
	public reward2: FireworkerItemRender;
	public reward3: FireworkerItemRender;
	...
	private config: ActivityType9Config | PActivity9Config;
	...
	private setCurAwardPage(): void {
		...
		if (this.actType == ActivityType.Normal) {
			...
			data = PActivity.ins().activityData[this.activityID] as PActivityType9Data;
		}
	...
	public updateData(): void {
		...
		if (data) {
			...
			let notice: any;
			for (let i = 0; i < len; i++) {
				let config: ActivityType9Config | PActivity9Config = config9[this.activityID][data.noticeArr[i].index];
				notice = {
	...
	private updateAwardPage(): void {
		...
		let config: ActivityType9Config | PActivity9Config = config9[this.activityID][0];
	...
	private resultCallBack(id: number) {
		...
		if (data.indexs.length > 1)//10连抽返回 弹出奖励界面
			ViewManager.ins().open(LuckyResultWin, this.activityID, data.indexs);

		this.updateMaterial();
	}

	private flyItemEx(itemicon: FireworkerItemRender) {
