class OSATarget9Panel1 extends BaseView{
    ...
    public updateData():void{
        ...
        let configList:ActivityType9Config[][]|PActivity9Config[][];
        if( this.actType == ActivityType.Normal ){
        ...
        for ( let i in configList[this.activityID] ) {
            if( !(+i) )continue;
            let cfg:ActivityType9Config|PActivity9Config = configList[this.activityID][i];
    ...
    private getLottery(): void {
        ...
        let ins:Activity|PActivity;
        let cfg:ActivityType9Config[]|PActivity9Config[];
        if( this.actType == ActivityType.Normal ){
            ins = Activity.ins();
    ...
    private updateProgress(){
        ...
        let config:ActivityType9Config|PActivity9Config = cfg[0];
        if( config && data ){
