class OSATarget11Panel1 extends BaseView{
    ...
    public updateData():void{
        ...
        if( !data )return;
        let len:number = Object.keys(GlobalConfig.ActivityType11_1Config[this.activityID]).length;
		let state:number = 0;
		let list:any[] = [];
		list.length = len;
		...
		let config2:ActivityType11_2Config;
		list = [];
		list.length = len;
		let total:number = 0;
		for (i = 0; i < len; i++)
