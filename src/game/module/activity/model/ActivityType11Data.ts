...
class ActivityType11Data extends ActivityBaseData {
	...
	public checkRedPoint(): boolean {
		...
		if (cfg.pageStyle == ActivityPageStyle.HAPPYSEVENDAY || cfg.pageStyle == ActivityPageStyle.SPRINGEIGHTDAY || cfg.pageStyle == ActivityPageStyle.VERSIONCELE) {
			let len: number = Object.keys(GlobalConfig.ActivityType11_2Config[this.id]).length;
			let conf: ActivityType11_2Config;
	...
	private checkPhaseAward(): boolean {
		let len: number = Object.keys(GlobalConfig.ActivityType11_1Config[this.id]).length;
		for (let i: number = 1; i <= len; i++) {
			if ((this._rewardInfo >> i & 1) <= 0 && this._hiScore >= GlobalConfig.ActivityType11_1Config[this.id][i].score) //可领取
				return true;
		}
