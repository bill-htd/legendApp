...
class ActivityType12Data extends ActivityBaseData {
    ...
    private envelopeSum = ActivityType12Data.maxEnvelope;//检测最大红包数
    ...
    public checkRedPoint(): boolean {
        let config: ActivityType12Config[] = GlobalConfig.ActivityType12Config[this.id];
        for (let k in config) {
...
class RedEnvelope {
    ...
    public startimer: number;//红包结束时间 2010时间戳
    public constructor() {
        this.id = 0;
        this.timer = 0;
    }
