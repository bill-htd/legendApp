...
class ActivityType3Data extends ActivityBaseData {
	...
	public getCurIndex(type?:number) {
		...
		let day = this.curOpenDay();//活动已开始天数
		// if (actConfig.timeType == 0) {//开服时间
		// 	let time = +actConfig.startTime.split("-")[0];
		// 	day = GameServer.serverOpenDay - time;
	...
	/**额外判定*/
	public canRewardEx():boolean{
		let config:ActivityType3Config[] = GlobalConfig.ActivityType3Config[this.id];
		if( config && config[1] && config[1].activityID ){//有嵌套活动就判定嵌套类型是否有红点
			for( let i = 0; i < config[1].activityID.length;i++ ){
				let actData:ActivityType3Data = Activity.ins().getActivityDataById(config[1].activityID[i]) as ActivityType3Data;
				if(actData && actData.canReward() )//嵌套的活动类型判定
					return true;
			}
		}
		return false;
