...
class XunzhangPanel extends BaseEuiView {
	...
	/**勋章开启相关 */
	private showAct: eui.Group;//勋章界面信息界面
	private Activation: eui.Group;//开启激活/勋章界面

	private UplevelBtn1: eui.Button;//开启勋章按钮
	...
	private showActPanel() {
		this.Activation.visible = false;//激活勋章界面
		this.showAct.visible = !this.Activation.visible;//勋章信息界面
	}
	/**更新 */
	private update(map?: any): void {
		...
		if (lv == 0) {
			...
			if (boo) {
				...
				if (!this.eff.parent) this.showAct.addChild(this.eff);
			} else {
				DisplayUtils.removeFromParent(this.eff);
			}
		}
