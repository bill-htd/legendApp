...
class UserFb extends BaseSystem {
	...
	//巅峰赛季
	public static FB_TYPE_PEAKED: number = 30;
	//暗之秘境boss
	public static FB_TYPE_DARK_BOSS: number = 36;
	//隐藏boss
	public static FB_TYPE_HIDE_BOSS: number = 37;
	//魂兽副本
	public static FB_TYPE_HUN_SHOU: number = 38;
	/** 组队副本 */
	 ...
	 * @type {number}
	 */
	public static FB_TYPE_PEAK: number = 30;
	...
	/** 腊八boss副本*/
	public static FB_TYPE_LABA: number = 31;
	...
	public showAutoPk2: number = -1;
	//开启自动闯关等级
	public static AUTO_GUANQIA: number = 20;
	public firstAutoGuilder: boolean = false;//自动闯关引导(只触发一次)
	 ...
	 */
	public fbRings: { buyTime: number, challengeTime: number, canTakeAward: boolean } = {
	...
	public constructor() {
		...
		this.regNetMsg(10, this.postFbInfoInit);
		this.regNetMsg(11, this.postUpDataInfo);
		...
		this.regNetMsg(25, this.postGuardInfo);
		this.regNetMsg(27, this.postGuardUseSkill);
		this.regNetMsg(28, this.postGuardCopyInfo);
		this.regNetMsg(29, this.postBossDrop);
		this.regNetMsg(30, this.postCreateTFRoomSuccess);
		this.regNetMsg(31, this.postEnterTFRoomSuccess);
		this.regNetMsg(32, this.exitTFRoom);
		this.regNetMsg(33, this.postFTRoomChange);
		this.regNetMsg(34, this.postFTRoomPassInfo);
		this.regNetMsg(35, this.doTeamFuBenEnd);
		this.regNetMsg(36, this.doTeamFuBenRelive);
		this.regNetMsg(37, this.postTeamFuBenRank);
		this.regNetMsg(38, this.postTeamFbFlowarRecords);
		this.regNetMsg(39, this.teamFbSysInviteInfo);
		this.regNetMsg(40, this.postGuardWeaponLogs);
	...
	private onChangeScene() {
		...
		// if (GameMap.fubenID > 0)//副本打开倒计时面版，哪些副本需要打开倒计时，界面里面判断
		// 	ViewManager.ins().open(FBCountDown);
	 ...
	 * @param bytes
	 */
	postFbInfoInit(bytes: GameByteArray): void {
	 ...
	 * 1-41
	 */
	public sendRingSweep(count: number): void {
	...
	private doBossResult(bytes: GameByteArray): void {
		...
		if (result) {
				...
				if (fbType == UserFb.FB_TYPE_GUIDEBOSS && GameMap.fubenID == 40000) {//全民boss引导副本
					...
					if (this.outPos.x && this.outPos.y) {
						...
						for (let i: number = 0; i < len; i++) {
							...
							if (char) GameMap.moveEntity(char, posX, posY);
						}
			...
			// if (GameMap.fbType == UserFb.FB_TYPE_MIJING) {//秘境
	...
	private doOfflineReward(bytes: GameByteArray): void {
		...
		for (let i = 0; i < len; i++) {
			...
			obj["type"] = bytes.readInt();//加成类型（1图鉴,2月卡,3符文,4新神器）
	 ...
	 *
	 */
	public sendTfFlower(id: number, count: number): void {
		let bytes: GameByteArray = this.getBytes(37);
		bytes.writeInt(id);
