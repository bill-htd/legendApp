...
class GameSceneView extends BaseEuiView {
	...
	public expTxt: egret.DisplayObjectContainer;
	...
	private leftTimePic: egret.DisplayObjectContainer;
	...
	private expChange(exp: number) {
		...
		let blood: egret.DisplayObjectContainer = new egret.DisplayObjectContainer();
		...
		if (this.leftTimeMj <= 0) {
			...
			for (let i: number = 0; i < GlobalConfig.GodWeaponBaseConfig.fbGrade.length; i++) {
				if (passNum <= GlobalConfig.GodWeaponBaseConfig.fbGrade[i]) {
					...
					UserFb.ins().mijingFingfen = i + 1;//秘境评分
		...
		switch (e.currentTarget) {
			...
			case this.escBtn:
				...
				if (BattleCC.ins().isBattle()) {
					WarnWin.show(`是否退出阵营副本，退出后有30秒进入CD`, () => {
						UserFb.ins().sendExitFb();
					}, this);
				...
				if (PaoDianCC.ins().isPaoDian) {
					WarnWin.show(`是否确定退出激情泡点？退出后将损失大量经验、威望奖励且有30秒进入CD`, () => {
						UserFb.ins().sendExitFb();
					}, this);
				...
				if (KfArenaSys.ins().isKFArena) {
					WarnWin.show(`战场未结束，退出将扣除大量竞技场积分，是否退出？`, () => {
						UserFb.ins().sendExitFb();
					}, this);
