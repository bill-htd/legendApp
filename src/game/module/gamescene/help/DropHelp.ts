...
class DropHelp {
	...
	static start(entity?: CharMonster): void {
	...
	private static findDrop(): void {
		let role: CharRole = this.mainEntity;
		...
		if (mylist.length > 1) {
			...
			for (let j = 0; j < mylist.length; j++) {
				...
				if (element.infoModel.masterHandle) {
					let ms = EntityManager.ins().getEntityByHandle(element.infoModel.masterHandle);
					if (ms) {
						selfMaster = ms;
					}
				}
				...
				let p = DirUtil.getGridByDir(selfMaster.dir + dirs[index] != null ? dirs[index] : 0);
				index += 1;
				//无目标时跟随主角
			...
			if (MathUtils.getDistance(role.x, role.y, target.x, target.y) < GameMap.CELL_SIZE >> 1) {
				...
				egret.Tween.get(target).wait(100).call(() => {
					if (target && target.parent) {
						let p = target.parent.localToGlobal(target.x, target.y);
