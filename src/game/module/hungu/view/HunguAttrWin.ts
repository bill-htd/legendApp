 ...
 *
 */
class HunguAttrWin extends BaseEuiView {
	...
	private setRoleAttr(roleId: number): void {
		...
		let value: number = 0;
		let i: number = 0;
		let attrsList:AttributeData[] = Hungu.ins().calcHunguTotalValue(role.index);
		for (let j: number = 0; j < this.proShowList.length; j++) {
			i = this.proShowList[j];
			value = 0;
			for( let idx = 0; idx < attrsList.length;idx++ ){
