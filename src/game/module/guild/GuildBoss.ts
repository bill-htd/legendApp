class GuildBoss extends BaseSystem {
	constructor() {
		...
		this.regNetMsg(1, this.postGuildBossInfoChange);
		this.regNetMsg(2, this.doRewardRecord);
		this.regNetMsg(3, this.postGuildBossDetailChange);
		this.regNetMsg(4, this.doGuildChallengeBack);
		this.regNetMsg(5, this.postGuildBossRankInfoChange);
	}
	 ...
	 * 46-1
	 */
	public postGuildBossInfoChange(bytes: GameByteArray): void {
	 ...
	 * 46-3
	 */
	public postGuildBossDetailChange(bytes: GameByteArray): void {
	...
	/**
	 * BOSS挑战返回
	 * 0.进入副本成功1.活动不开2.没有公会3.挑战次数不足4.已经通关所有boss5.正在有人挑战6.当前挑战关卡没配置7.服务器创建副本(配置)失败8.服务器创建怪物(配置)失败 99.直接扫荡返回
	 * 46-4
	 */
	 ...
	 * 46-5
	 */
	public postGuildBossRankInfoChange(bytes: GameByteArray): void {
	 ...
	 * 请求挑战BOSS
	 * 46-1
	 * @param type 0挑战 1一键挑战
	 */
	public sendChallengeBoss(type: number): void {
