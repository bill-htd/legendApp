...
class Guild extends BaseSystem {
	...
	public guildListInfos: GuildListInfo[] = [];//公会列表
	private _guildMembers: GuildMemberInfo[] = [];//公会成员列表
	public applyGuilds: number[] = [];//申请公会列表
	private _guillRoleSkillInfo: GuildRoleSkillInfo[] = [];//公会技能数据
	public guildTaskInfos: eui.ArrayCollection = new eui.ArrayCollection();//公会任务列表
	public guildMessageInfoData: eui.ArrayCollection//公会聊天记录包含公会公告
	...
	parserManage(bytes: GameByteArray): void {
		...
		//时间 unsigned int
		//事件类型 unsigned char 从1开始分别表示加入公会，离开公会，副会长任命，会长禅让，会长弹劾，公会副本进度首通，元宝/金币捐献，建筑升级
		//参数1 int
		//参数2 int
		//参数3 int
		//玩家名 string
