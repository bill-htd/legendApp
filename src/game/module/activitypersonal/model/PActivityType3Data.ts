 ...
 * 个人连续充值奖励数据
 * */
class PActivityType3Data extends ActivityBaseData {
	...
	{
		let configs: PActivity3Config[] = GlobalConfig['PActivity3Config'][this.id];
	...
	public curOpenDay(){
		let openTime = Math.floor(GameServer.serverTime - (DateUtils.formatMiniDateTime(this.startTime)) / 1000/3600/24);
		return openTime;//距离开启时间第几天( 有可能需要扩展成系统开启后第几天开 )
	}
	public getCurIndex(type?:number) {
		let actConfig = GlobalConfig.PActivityConfig[this.id];
		let type3Config:PActivity3Config[] = GlobalConfig.PActivity3Config[this.id];
		let day = 0;//活动已开始天数
		...
		if(this.startTime) {
			//距离开始的时间
			// let openTime = Math.floor(GameServer.serverTime - (DateUtils.formatMiniDateTime(this.startTime)) / 1000/3600/24);
			day = this.curOpenDay();//距离开启时间第几天( 有可能需要扩展成系统开启后第几天开 )
		}
		...
		configArr.sort((a:PActivity3Config,b:PActivity3Config)=>{
			if(a.day < b.day)
				return -1;
			return 1;
		});
	...
	getRewardStateById(index: number): number {
		let config: PActivity3Config = GlobalConfig.PActivity3Config[this.id][index];
		//判断是否有领取资格
