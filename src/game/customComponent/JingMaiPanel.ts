	...
	private setForgeData(levelUp: boolean = true): void {
		...
		if (stagesConfig.stage < GlobalConfig.JingMaiCommonConfig.stageMax) {
			...
			if (this._data.level > 0 && this._data.level % 10 == 0 && flag) {
				...
				this.countLabel.textFlow = TextFlowMaker.generateTextFlow(`消耗经脉丹:` + colorStr + count + "|/" + lvConfig.count);
			}
		...
		for (let i: number = 0; i < 9; i++) {
			(this["nadis" + i] as eui.Image).source = "jingmailine" + ((flag == 1 || (i + 1 < this._data.level % 10) ? "1" : "2"));
		}
