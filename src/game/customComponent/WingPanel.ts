...
class WingPanel extends BaseView {
	...
	//翅膀开启的表现
	public dor1: eui.Image;
	public dor2: eui.Image;
	public wing: eui.Group;
	...
	private updateLevel(): void {
		...
		for (let i: number = 0; i < 5; i++) {
			if (wingSkills[i] && wingSkills[i] > 0) {
				...
				let cfg: WingLevelConfig = GlobalConfig.WingLevelConfig[level];
	...
	private autoUpStar(): void {
		...
		let count: number = UserBag.ins().getBagGoodsCountById(0, config.itemId);
	...
	private notOpenStatus(): void {
		...
		let flag: boolean = GlobalConfig.WingCommonConfig.openLevel > Actor.level;
	...
	private setTimeDown(): void {
		...
		if (config.clearTime) {
			...
			if (this._wingsData.clearTime > 0) {
				...
				// this.lastTimeDown = (DateUtils.formatMiniDateTime(this._wingsData.clearTime) - GameServer.serverTime) / 1000;
	...
	private updateAtt(): void {
		...
		if (this._wingsData && this._wingsData.exp && nextLvConfig) {
			// let tempValue: number = Math.ceil(this._wingsData.exp / config.exp * 100);
		...
		// let len: number = this._wingsData.equipsLen;
		// for (let i: number = 0; i < len; i++) {
		// 	let equip: EquipsData = this._wingsData.getEquipByIndex(i);
		// 	if (equip.item.itemConfig)
		// 		power += ItemConfig.calculateBagItemScore(equip.item);
		// }
	...
	private isShowUpGradeBtn(): void {
		...
		if (this._wingsData.lv >= Wing.WingMaxLv) {
			...
			let count: number = UserBag.ins().getBagGoodsCountById(0, this.danItemID);
	...
	private showOpenMovie(): void {
		...
		// this.rapetNum = 0;
		// let tween: egret.Tween = egret.Tween.get(this.wing, { "loop": true });
		if (!this.flyUpEffect) {
		...
		// this.addChild(this.flyUpEffect);
		// this.swapChildren(this.flyUpEffect,this.wingL);
		...
		// masksp.mask = this.flyUpEffect;
		// square.mask = this.flyUpEffect;
		...
		tween0.to({ "y": 0 }, 1500).call(() => {
			...
			// this.removeChild(this.flyUpEffect);
		...
		// tween.to({ "y": 194 }, 50).to({ "y": 214 }, 50).call(() => {
		// 	++this.rapetNum;
		// 	if (this.rapetNum == 7) {
		// 		this.flowReckEffect();
		// 	}
		// 	if (this.rapetNum >= 10) {
		// 		egret.Tween.removeTweens(this.wing);
	...
	private flowReckEffect(): void {
		...
		TimerManager.ins().doTimer(500, 1, () => {
			...
			let tween0: egret.Tween = egret.Tween.get(this.openEffect, {
				loop: false, onChange: () => {
					let tween1: egret.Tween = egret.Tween.get(this.dor1);
					let tween2: egret.Tween = egret.Tween.get(this.dor2);
					let tween3: egret.Tween = egret.Tween.get(this.shan);
					tween1.to({ "x": -280 }, 500);
					tween2.to({ "x": 558 + 280 }, 500).call(() => {
						Wing.ins().sendActivate(this.curRole);
					}, this);
					this.wing.visible = false;
					tween3.to({ alpha: 0 }, 500);
					DisplayUtils.removeFromParent(this.openEffect);
