...
// } else {
//     alert('跳转失败，请重新重第三方登录跳转')
// }
//对数据进行判断
if ((!urlParam.user || !urlParam.srvid || !urlParam.srvaddr || !urlParam.srvport) && !isTokenLogin) {
    var ver = '1.0.0'
    var backIndex = paraUrl.split('/');
    backIndex[backIndex.length - 1] = 'backIndex.html';
    backIndex = backIndex.join('/');
    document.body.innerHTML += '<iframe id="paramIframe" style="border: 0px; width: 100%;height: 100%;" ' +
        'src="./login.html?ver_res=' + ver + '"></iframe>';
}
// else {
//     if (token) {
//         document.body.innerHTML += '<iframe id="paramIframe" style="border: 0px; width: 100%;height: 100%;" ' +
...
function getChargeUrl() {
    return 'http://winyso.com/gm/index.php?m=Payment&a=Placeorder';
}
function getChatControlUrl() {
    return 'http://47.75.114.116/gm/index.php?m=Chat&a=chat_controls&';
}
function getkefuUrl() {
    return 'https://chat-new.mqimg.com/widget/standalone.html?eid=184645';
}

// createIframe('https:www.baidu.com')
